...
[![Paper](https://img.shields.io/badge/%F0%9F%93%84-Paper-red)](https://openreview.net/pdf?id=GKvb3HCyNk)
...
Download pretrained weights from the [HuggingFace repository](https://huggingface.co/alistairfoggin/CroCoDiLight)
...
|------|-------------|-------------|
| [`CroCoDiLight.pth`](https://huggingface.co/alistairfoggin/CroCoDiLight/resolve/main/CroCoDiLight.pth) | All inference scripts | Full CroCoDiLight model (includes the CroCo encoder, single-view decoder, lighting extractor, and lighting entangler) |
| [`CroCoDiLight_shadow_mapper.pth`](https://huggingface.co/alistairfoggin/CroCoDiLight/resolve/main/CroCoDiLight_shadow_mapper.pth) | Shadow removal | Lighting mapper trained for shadow removal |
| [`CroCoDiLight_albedo_mapper.pth`](https://huggingface.co/alistairfoggin/CroCoDiLight/resolve/main/CroCoDiLight_albedo_mapper.pth) | Albedo estimation | Lighting mapper trained for intrinsic image decomposition |
...
|------|---------|-------------|
| [`CroCo_V2_ViTLarge_BaseDecoder.pth`](https://download.europe.naverlabs.com/ComputerVision/CroCo/CroCo_V2_ViTLarge_BaseDecoder.pth) | Steps 1 & 2 | Original CroCo v2 ViT-Large weights from [Weinzaepfel et al.](https://github.com/naver/croco) (includes the cross-view decoder, though only the encoder is used). |
| [`CroCoDiLight_decoder.pth`](https://huggingface.co/alistairfoggin/CroCoDiLight/resolve/main/CroCoDiLight_decoder.pth) | Step 2 | Single-view decoder pretrained on ImageNet reconstruction (output of training step 1). Available for download but not needed for inference as its weights are embedded in `CroCoDiLight.pth`. |
...
The API accepts ImageNet-normalized `float32` tensors in `N, C, H, W` layout.
Use `load_image` for image paths or `pil_to_tensor` for PIL images:

```python
from crocodilight import (
    apply_mapper, extract_lighting, load_mapper, load_model, relight, load_image
)

model = load_model("pretrained_models/CroCoDiLight.pth", device="cuda:0")

reference_image = load_image("/path/to/reference/image.png", device="cuda:0")
other_image = load_image("/path/to/other/image.png", device="cuda:0")

lighting = extract_lighting(model, reference_image) # Extracts lighting for a single image
relit_image = relight(model, other_image, lighting) # Can relight a batch of images

shadow_mapper = load_mapper(
    model, "pretrained_models/CroCoDiLight_shadow_mapper.pth", device="cuda:0"
)
shadow_free = apply_mapper(model, other_image, shadow_mapper) # Can apply it to a batch of images
```

Use the albedo mapper with `apply_mapper` for albedo estimation. Lighting swaps can
be composed by extracting each image's lighting and relighting the opposite image.
...
Agentic coding tools were used to package the project as an installable Python module
(pyproject.toml), refactor inference scripts (removing hard-coded paths and extracting
shared model loading and utility functions), and build the Gradio demos. All core research
code, model architectures, and training procedures were designed and written by the authors.
